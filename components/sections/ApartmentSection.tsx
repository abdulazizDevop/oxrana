...
import { Plus, Home, Loader2, Trash2, X } from "lucide-react";
import FileUpload from "@/components/FileUpload";

type Record = { id: string; data: { room_name: string; issue?: string; status: string; notes?: string; }; files?: { id: string; url: string; file_name: string; file_type: string; file_size: number }[]; created_at: string; };

const statusColor = (s: string) => s === "ok" ? "text-emerald-400" : s === "issue" ? "text-red-400" : "text-amber-400";
const statusLabel = (s: string) => s === "ok" ? "Норма" : s === "issue" ? "Проблема" : "Проверка";
const statusBg = (s: string) => s === "ok" ? "bg-emerald-500/10" : s === "issue" ? "bg-red-500/10" : "bg-amber-500/10";

export default function ApartmentSection({ city, companyId }: { city: string; companyId?: string }) {
  const [records, setRecords] = useState<Record[]>([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ room_name: "", issue: "", notes: "", status: "ok" });
  ...
  async function fetchAll() {
    setLoading(true);
    const q = new URLSearchParams({ section: "apartment" });
  ...
  async function openForm() {
    const res = await fetch("/api/records", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ cityId: city, companyId: companyId || "", section: "apartment", data: {} }) });
  ...
  async function save() {
    ...
    setForm({ room_name: "", issue: "", notes: "", status: "ok" });
  ...
  async function cancelForm() {
    ...
    setPendingId(null); setShowForm(false); setForm({ room_name: "", issue: "", notes: "", status: "ok" });
  }
  ...
  const filteredRecords = records.filter(r => {
    ...
    return r.data.room_name?.toLowerCase().includes(s) || 
           r.data.issue?.toLowerCase().includes(s) || 
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="search-wrap">
        ...
        <input
          ...
          type="text"
          placeholder="Поиск по помещению или проблеме..."
          value={searchQuery}
          ...
          <div style={{ position: "fixed", inset: 0, zIndex: 400, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
            ...
            >
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                <div>
                  <h3 style={{ fontSize: 17, fontWeight: 700, color: "#f0f0fa", margin: 0 }}>Новая запись</h3>
                  <p style={{ fontSize: 12, color: "#555570", margin: "4px 0 0" }}>Укажите данные помещения</p>
                </div>
              ...
              <div className="flex gap-2 pt-4">
                <motion.button onClick={save} disabled={saving} whileHover={{ scale: saving ? 1 : 1.02 }} whileTap={{ scale: saving ? 1 : 0.97 }}
                  className="flex-1 py-4 bg-gradient-to-br from-[#4f8ef7] to-[#2563eb] text-white rounded-2xl text-[15px] font-semibold flex items-center justify-center gap-2 shadow-lg shadow-[#4f8ef7]/30 hover:from-[#5a96f8] hover:to-[#2d6ee5] transition-all">
      ...
      {loading ? (
        ...
        <div className="text-center py-14 text-white/20 flex flex-col items-center gap-3">
          ...
          <span className="text-sm">Записей не найдено</span>
        </div>
      ) : (
            ...
            <motion.div key={r.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ delay: i * 0.04 }}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <div className="w-9 h-9 rounded-xl bg-green-500/10 flex items-center justify-center flex-shrink-0"><Home size={16} className="text-green-400" /></div>
                  <div className="min-w-0">
                    <p className="text-white text-sm font-medium truncate">{r.data.room_name}</p>
                    <p className="text-white/30 text-xs truncate">{r.data.issue || r.data.notes || "—"}</p>
                  </div>
                </div>
