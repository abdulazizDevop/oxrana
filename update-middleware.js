...
function applySecurityHeaders(response: NextResponse, nonce?: string, cspHeader?: string) {
  if (cspHeader) {
...
export async function middleware(request: NextRequest) {
  ...
  if (['POST', 'PUT', 'DELETE', 'PATCH'].includes(request.method)) {
    ...
    if (origin && host) {
      ...
      if (originUrl.host !== host) {
        return applySecurityHeaders(NextResponse.json({ error: 'CSRF token mismatch or invalid origin' } , { status: 403 }));
      }
    }
  }
  ...
  if (pathname.startsWith('/api')) {
    ...
    if (!token) {
      return applySecurityHeaders(NextResponse.json({ error: 'Unauthorized: No token provided' } , { status: 401 }));
    }
    ...
    try {
      ...
      if (!isAdmin && adminOnlyPrefixes.some(prefix => pathname.startsWith(prefix))) {
        return applySecurityHeaders(NextResponse.json({ error: 'Forbidden: Admin access required' }, { status: 403 }));
      }
      ...
      if (pathname.startsWith('/api/users') && !isAdmin && ['POST', 'PUT', 'DELETE'].includes(method)) {
         if (!allowedSections.includes('employees')) {
            return applySecurityHeaders(NextResponse.json({ error: 'Forbidden: Employees access required' }, { status: 403 }));
         }
      }
      ...
      return applySecurityHeaders(NextResponse.json({ error: 'Unauthorized: Invalid token' } , { status: 401 }));
    }
  }
...
fs.writeFileSync(path.join(__dirname, 'rasul/Desktop/security-tracker/middleware.ts'), content);
